...
class KUAIRECDataLoader(object):
    """
    Load KUAIRANK16DataLoader for torch train/eval
    ...
    def __init__(self, dataset_name, dataset_path, device, bsz=32):
        ...
        for key, df in data.items():
            if key == 'description':
                continue
            self.dataloaders[key] = DataLoader(KUAIRECDataset(dataset_name, df, self.description, device), batch_size=bsz, shuffle=False)
