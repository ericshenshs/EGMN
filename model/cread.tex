...
\title{\texttt{\detokenize{model/cread.py}} (Q\&A Documentation)}
\author{}
\date{}
\begin{document}
\begin{CJK*}{UTF8}{gbsn}
...
\textbf{A:} This file implements the CREAD baseline model: a multi-head network that outputs a vector of probabilities corresponding to discretized thresholds of watch-time.

\paragraph{Q: Why does CREAD output many heads instead of one scalar?}
\textbf{A:} The training script discretizes watch-time into multiple binary comparisons against split nodes. Predicting all thresholds makes the model learn an ordinal structure that can be converted back into a scalar via weighted bucket sizes.
...
class Cread  # methods: __init__, build, init, forward
\end{Verbatim}
...
\vspace{1mm}
\VerbatimInput[firstline=1,lastline=50,fontsize=\small]{/workspace/model/cread.py}


\paragraph{Q: What is \texttt{\detokenize{Cread}} and what responsibility does it have?}
...
\paragraph{Q: What are the important methods on \texttt{\detokenize{Cread}}?}
...
\paragraph{Q: What does the implementation look like for \texttt{\detokenize{Cread}}?}
\textbf{A:} Excerpt from the file:

\vspace{1mm}
\VerbatimInput[firstline=5,lastline=58,fontsize=\scriptsize]{/workspace/model/cread.py}


\paragraph{Q: Why is \texttt{\detokenize{Cread}} needed by training scripts?}
...
\VerbatimInput[fontsize=\scriptsize]{/workspace/model/cread.py}
