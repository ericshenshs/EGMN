...
\title{\texttt{\detokenize{model/wd.py}} (Q\&A Documentation)}
\author{}
\date{}
\begin{document}
\begin{CJK*}{UTF8}{gbsn}
...
\textbf{A:} This file implements a Wide \textbackslash\{\}\& Deep baseline: an embedding+MLP deep part, optionally combined with a summed linear contribution from continuous features (wide part).

\paragraph{Q: Why is this baseline needed?}
\textbf{A:} Wide \textbackslash\{\}\& Deep is a standard recommender baseline and acts as a reference point when judging distributional models like EGMN.
...
class WideAndDeep  # methods: __init__, build, init, forward
\end{Verbatim}
...
\vspace{1mm}
\VerbatimInput[firstline=1,lastline=50,fontsize=\small]{/workspace/model/wd.py}


\paragraph{Q: What is \texttt{\detokenize{WideAndDeep}} and what responsibility does it have?}
...
\paragraph{Q: What are the important methods on \texttt{\detokenize{WideAndDeep}}?}
...
\paragraph{Q: What does the implementation look like for \texttt{\detokenize{WideAndDeep}}?}
\textbf{A:} Excerpt from the file:

\vspace{1mm}
\VerbatimInput[firstline=5,lastline=58,fontsize=\scriptsize]{/workspace/model/wd.py}


\paragraph{Q: Why is \texttt{\detokenize{WideAndDeep}} needed by training scripts?}
...
\VerbatimInput[fontsize=\scriptsize]{/workspace/model/wd.py}
