...
\title{\texttt{\detokenize{model/tpm.py}} (Q\&A Documentation)}
\author{}
\date{}
\begin{document}
\begin{CJK*}{UTF8}{gbsn}
...
\textbf{A:} This file implements TPM-style output: the model predicts probabilities for a binary tree over buckets, which can be decoded into an expected watch-time.

\paragraph{Q: Why does TPM return a vector instead of a scalar?}
\textbf{A:} TPM represents the label as a structured set of binary decisions (tree nodes). This provides many supervised signals per sample and enables decoding into an expected value and an uncertainty proxy.
...
\vspace{1mm}
\VerbatimInput[firstline=1,lastline=50,fontsize=\small]{/workspace/model/tpm.py}


\paragraph{Q: What is \texttt{\detokenize{TPM}} and what responsibility does it have?}
...
\paragraph{Q: What are the important methods on \texttt{\detokenize{TPM}}?}
...
\paragraph{Q: What does the implementation look like for \texttt{\detokenize{TPM}}?}
\textbf{A:} Excerpt from the file:

\vspace{1mm}
\VerbatimInput[firstline=5,lastline=52,fontsize=\scriptsize]{/workspace/model/tpm.py}


\paragraph{Q: Why is \texttt{\detokenize{TPM}} needed by training scripts?}
...
\VerbatimInput[fontsize=\scriptsize]{/workspace/model/tpm.py}
